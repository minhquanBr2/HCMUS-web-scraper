...
def get_parameters(table_name):
    ...
    for row in parameter_names:
        ...
        if parameter_name.lower() in ["chip", "name", "info_link", "id", "img_link", 'gpu', 'type', 'shop']:
            continue
...
def createSQL(table_name, filters, sort_type):
    ...
    sql = select + f""" FROM {table_name} WHERE name IS NOT NULL AND price IS NOT NULL AND info_link IS NOT NULL AND img_link IS NOT NULL """
...
def findProductById(table_name: str, index: str):
    ...
    sql = select + f""" FROM {table_name} WHERE name IS NOT NULL AND price IS NOT NULL AND info_link IS NOT NULL AND img_link IS NOT NULL AND id = {index} """
    cursor.execute(sql)
    row = cursor.fetchone()
    ...
    sql = select + f""" FROM {table_name} WHERE name IS NOT NULL AND price IS NOT NULL AND info_link IS NOT NULL AND img_link IS NOT NULL AND id <> {index}  AND brand LIKE  '%{product['brand']}%' """ + conditional + f""" AND MATCH(name) AGAINST('{against}') LIMIT 15 """
    print(conditional)
    cursor.execute(sql)
    rows = cursor.fetchall()
...
def search_product(name: str,  page_index: int = 1, product_per_page: int = 20, sort_type: int = 1):
    ...
    WHERE name LIKE %s AND price IS NOT NULL AND img_link IS NOT NULL AND info_link IS NOT NULL 
    """
